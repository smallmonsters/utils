		...
		<view class="popup-content">
			<view class="popup-content-main">
                ...
                <view class="picker__content">
					<input class="uni-input bottom-line keyword-input" v-if="isSearch" v-model="searchVal" placeholder="输入关键字搜索" @input="search"/>
                    <view class="check__all" v-if="multiple">
						<view class="check__all_left">全选</view>
                        <view class="check__all__right">
							<checkbox v-model="checkedAll" name="all" @click="toggleAll" :checked="checkedAll" style="transform:scale(0.8)"/>
                        </view>
                    </view>
					...
					<view class="uni-list" v-if="multiple">
						<checkbox-group @change="checkboxChange" v-if="columnsData.length>0">
							<label class="uni-list-cell uni-list-cell-pd justify-between" v-for="item in columnsData" :key="item[option.value]">
								<view class="cell-label">{{item[option.label]}}</view>
								<view>
									<checkbox :value="item[option.value]" :checked="checkboxValue.includes(item[option.value])" style="transform:scale(0.8)"/>
								</view>
							</label>
						</checkbox-group>
						<view v-else>
					...
					<view class="uni-list" v-else>
						<radio-group @change="checkboxChange" v-if="columnsData.length>0">
							<label class="uni-list-cell uni-list-cell-pd justify-between" v-for="item in columnsData" :key="item[option.value]">
								<view class="cell-label">{{item[option.label]}}</view>
								<view>
									<radio :value="item[option.value]" :checked="selectValue.includes(item[option.value])" style="transform: scale(0.8);"/>
								</view>
							</label>
						</radio-group>
						<view v-else>
...
<script>
  export default {
    ...
    watch: {
      ...
      columnsData: {
        handler (val) {
			 console.log('watch-------columnsData:',val)
      ...
      checkboxValue: {
        handler (val) {
			console.log('watch-------checkboxValue:',val)
